  ...
  <div class="scroll-bar">
    ...
    <div class="scroll-bar">
      ...
      <el-scrollbar height="80vh">
        <el-card v-for="item,index in noteData" :key="currentIndex" class="scroll-item" @click="handleClick(index)">
          <div>
            {{ item.noteTitle }}

          </div>
          <span class="scroll-ctime" v-if="item.modifiTime==null">创建时间:{{ item.craeteTime }}</span>
          <span class="scroll-ctime" v-if="item.modifiTime!=null">修改时间:{{ item.modifiTime }}</span>
        </el-card>
      </el-scrollbar>
...
export default {
  ...
  data() {
    ...
    changeDiologVisb: function () {
      ...
      this.$emit('handle-btn-click',this.formname)
    }
  }
}
</script>
